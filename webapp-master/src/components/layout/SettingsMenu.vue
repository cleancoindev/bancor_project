  ...
  >
    ...
    <b-dropdown-group id="dropdown-group-1" v-if="showTx && !showLocale">
      <b-dropdown-text :variant="darkMode ? 'dark' : 'light'">
        <span class="font-size-sm">{{ $t("slippage_tolerance") }}</span>
        <slippage-tolerance />
      </b-dropdown-text>
    </b-dropdown-group>
    <div
      class="border-top-menu mt-2""
      :class="darkMode ? 'border-top-menu-dark' : 'border-top-menu-light'"
    ></div>
    <b-dropdown-group id="dropdown-group-2" v-if="!showLocale" class="mt-2 mb-2">
      <b-dropdown-text :variant="darkMode ? 'dark' : 'light'">
        <MenuToggle
          :label="$t('dark_mode')"
          :value="darkMode"
    ...
    <div
      ...
      :class="darkMode ? 'border-top-menu-dark' : 'border-top-menu-light'"
    ></div>
    <b-dropdown-group id="dropdown-group-3" v-if="!showLocale" class="mt-3">
